...
def sample(tpl, V):
    x = torch.tensor(components.encode_token(V, tpl[0]), dtype=torch.long)
    y = torch.tensor(components.encode_token(V, tpl[1]), dtype=torch.long) 
    return x, y
...
def generate_data(V):
    ...
    for i in range(grp_len):
        test_samples.append( ( lowercases[i] + rep_chars[1], lowercases[i] ) )
    for i in range(grp_len):
